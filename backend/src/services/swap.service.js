...
export async function getExactTokenInSwapQuote(chain, protocol, tokenIn, tokenOut, amountIn, TokenList) {
...
export async function getExactTokenOutSwapQuote(chain, protocol, tokenIn, tokenOut, amountOut,TokenList) {
  const provider = getProvider(chain);
  const routerAddr = addresses[protocol]?.[chain]?.exchange_router;
   if (!routerAddr) throw new Error("Router not configured");
  const exchange_router = new Contract(routerAddr, ExchangeRouterAbi, provider);
 ...
 // building the best path for swap 
  ...
  try {
    ...
    const amountsIn = await exchange_router.getAmountsIn(BigInt(amountOut), path);
...
// exact swaps after preview i.e also after finnest path selction.

export async function getExactTokenInSwap(chain, protocol,amountIn,slippage,path,deadline){
  const provider = getProvider(chain);
...
try {
  ...
  const amountOut = await exchange_router.swapExactTokensForTokens(amountIn, slippage, path,recipient,deadline);

  return {
    amount : amountOut[amountOut.length() -1]
  };
   } catch (err) {
    throw new Error("Swap not Succeded");
  }
}


export async function getExactTokenOutSwap(chain, protocol,amountOut,slippage,path,deadline){
  const provider = getProvider(chain);
  const signer = provider.getSigner();
  const routerAddr = addresses[protocol]?.[chain]?.exchange_router;
   if (!routerAddr) throw new Error("Router not configured");
...
try {
  const recipient = await signer.getAddress();
  const amountIn = await exchange_router.swapTokensForExactTokens(amountOut, slippage, path,recipient,deadline);
